...
const Formulario = () => {
  ...
  const UpdateTagState = (field, value) => {
    setState((prevState) => {
      const {
        ...
        protocoloEmergencialBoolean,
        protocoloEmergencialNome,
      } = checkProtocoloEmergencial(field, value);
      return {
        ...prevState,
        tagStates: {
          ...prevState.tagStates,
  ...
  const checkProtocoloEmergencial = (field, value) => {
    if (field === "vitimas") {
      ...
      if (value.estadoVitima === "Inconsciente") {
        return {
          ocorrenciaEmergencial: true,
          protocoloEmergencialBoolean: true,
          protocoloEmergencialNome: "PCR",
        };
      }
    } else if (field === "ovace") {
      if (value.confirmaOVACE) {
        return {
          ocorrenciaEmergencial: true,
          protocoloEmergencialBoolean: true,
          protocoloEmergencialNome: "OVACE",
        };
      }
    } else if (field === "incendio_vegetacao") {
      if (
        value.pessoasBoolean ||
        value.animaisBoolean ||
        value.residenciasBoolean
      ) {
        return { ocorrenciaEmergencial: true };
      }
  ...
  const sortedOptions = naturezasList.sort((a, b) =>
  ...
  function gerarNarrativa() {
    ...
    if (incidenteBoolean) {
      ...
      narrativa += `OCORRÊNCIA DE ${naturezas
        .map((item) => item.nome.toUpperCase())
        .join(", ")} NÃO ATENDIDA PELO CBMDF`;
    } else {
  ...
  function adicionarProtocoloEmergencial() {
    return protocoloEmergencialBoolean
      ? `## PROTOCOLO DE ${protocoloEmergencialNome} INICIADO PELA CENTRAL DE OPERAÇÕES  ##\n\n`
      : "";
  }

  function adicionarTransferenciaForaDF() {
    return ufOcorrencia !== "DF"
      ? `Ocorrência fora da circunscrição\nTransferida para ${cidadeOcorrencia} - ${ufOcorrencia}\n\n`
      : "";
  }

  function adicionarInformacoesNaturezas() {
    return `Natureza(s): ${naturezas
      .map((item) => item.nome.toUpperCase())
      .join(", ")}\n`;
  }
  ...
  function adicionarRelatoEndereco() {
    ...
    if (ufOcorrencia === "DF") {
      ...
      texto += pontoReferencia
        ? `Ponto de Referência: ${pontoReferencia.toUpperCase()}\n`
        : "";
      texto += nomeSolicitante
        ? `Nome do Solicitante: ${nomeSolicitante.toUpperCase()}\n`
        : "";
    }
  ...
  function adicionarInformacoesVitima() {
    ...
    if (tags.includes("vitimas")) {
      ...
      texto += `- ${
        tagStates.vitimas.estadoVitima == "Outros"
          ? ". " + tagStates.vitimas.estadoVitimaOutroText
          : tagStates.vitimas.estadoVitima
      }\n`;
      if (tagStates.vitimas.estadoVitimaOutroEmergencialBoolean)
        texto += `- Quadro classificado como EMERGENCIAL pelo teleatendente\n`;
    }
    return texto;
  }
  ...
  function adicionarInformacoesIncendioVegetacao() {
    ...
    if (tags.includes("incendio_vegetacao")) {
      ...
      if (tagStates.incendio_vegetacao.pessoasBoolean)
        texto += `- Pessoas no local do incêndio\n`;
      if (tagStates.incendio_vegetacao.animaisBoolean)
        texto += `- Animais no local do incêndio\n`;
      if (tagStates.incendio_vegetacao.residenciasBoolean)
        texto += `- Incêndio ameaçando residências\n`;
      if (tagStates.incendio_vegetacao.origemBoolean)
        texto += `- Suposta origem informada do incêndio: ${tagStates.incendio_vegetacao.origemText}\n`;
      if (tagStates.incendio_vegetacao.irEstradaBoolean)
        texto += `- Solicitante informou que se deslocará para estrada a fim de facilitar a localização\n`;
    }
  ...
  function adicionarInformacoesCapturaInseto() {
    ...
    if (tags.includes("captura_inseto")) {
      ...
      if (tagStates.captura_inseto.insetosAtacandoBoolean)
        texto += `- Insetos atacando pessoas\n`;
      if (tagStates.captura_inseto.localInsetos)
        texto += `- Local dos insetos: ${tagStates.captura_inseto.localInsetos}\n`;
      if (tagStates.captura_inseto.alturaEnxame)
        texto += `- Altura aproximada do enxme: ${tagStates.captura_inseto.alturaEnxame}\n`;
    }
  ...
  const narrativa = useMemo(
    () => gerarNarrativa(),
    [state, naturezas, tags, tagStates]
  );
  ...
  return (
    <Grid>
      ...
      <SwitchField
        ...
        value={incidenteBoolean}
        onChange={(value) => handleChange("incidenteBoolean", value)}
      />
        ...
        !incidenteBoolean && (
          <Grid>
            ...
            <SwitchField
              ...
              value={naturezaAgenciaBoolean}
              onChange={(value) =>
                handleChange("naturezaAgenciaBoolean", value)
              }
            />
            {naturezaAgenciaBoolean && (
              <Grid>
              ...
              <Grid item xs={12}>
...

                {
                  ...
                  <Grid>
                    <AlertMessage
                      severity="info"
                      title='"De qual cidade está falando?"'
                    ></AlertMessage>
                    <UfSelect
                      value={ufOcorrencia}
                      title="UF"
                    ...
                    {ufOcorrencia !== "DF" && cidadeOcorrencia !== "" && (
                      <Grid>
                        <AlertMessage
                          severity="info"
                          title={`Você ligou para o Distrito Federal, mas vamos lhe transferir 
                        ...
                        <UnidadesTable
                          value={cidadeOcorrencia}
                          uf={ufOcorrencia}
                          cidade={cidadeOcorrencia}
                          title="Selecione a Unidade"
                          name="unidadeTransferência"
                          onChange={handleChange}
                        />
                        <AlertMessage
                          severity="error"
                        ...
                        {protocoloEmergencialBoolean && (
                          <a
                            href={`socorro_teleatendimento/${protocoloEmergencialNome}`}
                          >
                            <Button
                              size="large"
                              variant="contained"
                    ...
                    {ufOcorrencia === "DF" && cidadeOcorrencia !== "" && (
                      <Grid>
                        ...
                        <InputCopy
                          field="Endereço"
                          value={`${endereco.toUpperCase()} - ${cidadeOcorrencia.toUpperCase()} - ${ufOcorrencia}`}
                        />
                        <AlertMessage
                          severity="error"
                                  ...
                                  {!telefoneSolicitanteBoolean && (
                                    <Grid>
                                      ...
                                      <CustomTextField
                                        ...
                                        value={telefoneSolicitanteNumber}
                                        onChange={handleChange}
                                      />
                                    </Grid>
                                  )}
