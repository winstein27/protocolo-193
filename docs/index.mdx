...
graph TD;
    ...
    Coletar_cidade --> |Dentro do DF|Registrar_endereco(Digitar o endereço completo no campo LOGRADOURO):::Atendente
    Registrar_endereco --> 
...
```mermaid
graph TD
    B(Identificar órgão responsável pela ocorrência):::link
    B --> C["Senhor(a), essa ocorrência é atendida pelo ${ÓRGÃO}\nVamos transferir automaticamente sua ligação\n
    Pedimos que aguarde na linha e, caso a ligação caia, faça contato novamente"]
    C --> D("Transferir ligação para ${ÓRGÃO}"):::atendente
    D --> E(Registrar a situação e para qual órgão\nfoi transferida a ligação no campo NARRATIVA):::atendente
    E --> F(Classificar o atendimento como CHAMADA TRANSFERIDA\nno campo CLASSIFICAÇÃO DA CHAMADA):::atendente
    F --> G((Encerrar\nAtendimento))
    
    click B "#contatos-das-agências-externas" "Contatos das Agências Externas"
    classDef atendente fill:#d66, color:#fff
    classDef link fill:#33d, color:#fff
``` 
#### Contatos dos Quartéis do CBMDF

| Unidade | Local                                  | Contato |
|---------|----------------------------------------|---------|
| 1º GBM  | Brasília                               |3193-0001|

#### Contatos das Agências Externas
...
| RECOLHIMENTO DE ANIMAL MORTO |  - | (61) 3213-0178 | 
